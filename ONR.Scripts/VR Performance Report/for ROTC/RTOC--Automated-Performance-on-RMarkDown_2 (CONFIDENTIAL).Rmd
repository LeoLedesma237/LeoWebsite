...
cat("The following document is intended solely for ID # [THIS SECTION HAS BEEN REDACTED]")
```
...
GnG_Performance.mean <- GnG_Performance.mean %>%
  ...
  mutate(percent_rank = round(rank(Performance)/length(Performance),3)*100)
...
cat('\n The following bar graph indicates how well you performed compared to the other participants in the study (n=',length(unique(data$UserID)),"). These percentiles (not percentages) tell us that for Target trials on your Rest day, you performed better than roughly ",Nback.clean.day1$percent_rank[2],"% of participants and roughly worse than ", 100 - Nback.clean.day1$percent_rank[2],"%. The same logic applies to the other testing days and trial types.",sep="")  
...
ASVAB <- ASVAB %>%
  group_by(Subject) %>%
  mutate(percent_rank = round(rank(mean.percent)/length(mean.percent),3)*100)
