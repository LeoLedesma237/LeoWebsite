...
# Also merge to add IQ data
setwd("~/ONR/IQ_data/Processed Data")

# Load in the CFIT data
CFIT <- read.csv("CFIT.raw.scores.csv")

# Remove variable X from CFIT
CFIT <- CFIT %>% select(-X)


# Add IQ covariate- since it is not a predictor nor outcome variable, keep NA's for those missing CFIT data
GnG.performance.merged <- GnG.performance.merged %>%
  merge(CFIT, by = "ID", all.x = TRUE)

# Do the same for the Nback
...
For our graphs, if we were to exclude our outliers in the box plot for Inhibition and GnG then overall these data look to not violate the assumption of homoscedasiticty. Additionally this is further supported with a non significant result in the Breusch-Page test. The Working Memory and Nback plots box plots look to cover the same range with the exception of Q4, which is much more concentrated between .4 and .75. However, with the inclusion of a non significant Breusch-Pagan test, these data also do not violated the assumption of homoscedasticity.
...
```{r load in IQ data, warning = FALSE}
# Create a correlation matrix with our covariates for inhibition
Inhibition.covariates <- GnG.performance.merged %>%
  select(Age, CFIT) %>%
  unique() %>%
  filter(complete.cases(.))

# get the dimensions of our GnG covariates
dim(Inhibition.covariates)

# Run a correlation matrix
cor(Inhibition.covariates)

# Create a correlation matrix with our covariates for working memory
Working.Memory.covariates <- Nback.performance.merged %>%
  select(Age, CFIT) %>%
  unique() %>%
  filter(complete.cases(.))

# get the dimensions of our GnG covariates
dim(Working.Memory.covariates)

# Run a correlation matrix
cor(Working.Memory.covariates)

  
```

Our covariates do not correlate highly with each other- thus the assumption of independence was not violated. 


### Multiple Regression

We will now run two multiple regressions. We are not so interested in the output as of just yet, but more in the plots that can be derived from having created the model. These plots will further indicate if assumptions have been violated or not. 

Temporarily removed CFIT until we can identify more scores from manual data cleaning. 

```{r running multiple regression on working memory}
# Running a multiple regression 
# Run the GnG regression
No.Go.data <- GnG.performance.merged %>%
  filter(Trial.Type == "No-Go")
  
Inhibition.model <- lm(trial.mean.correct ~ T.score + Age,data = No.Go.data)

# Run the Nback regression
Target.data <- Nback.performance.merged %>%
  filter(Trial.Type == "Target")

Working.Memory.model <- lm(target.mean.reacted ~  T.score + Age, data = Target.data)

```

### Visualizing residuals from both models

```{r visualizing the residuals from the models, out.width = "110%"}
### First for Inhibiton
# Add the standardized residuals to our original dataframe
GnG.performance.merged$standardized.residuals <- rstandard(Inhibition.model)

# Add studentized residuals
GnG.performance.merged$studentized.residuals <- rstudent(Inhibition.model)

# Add the fitted (predicted) values to our original dataframe
GnG.performance.merged$fitted.values <- Inhibition.model$fitted.values

# Create a graph using the fitted values and the standardizes residuals
Inhibition.plot1 <- GnG.performance.merged %>%
  ...
  labs(title = "Standardized Residuals vs Fitted Values\n(Inhibition)",
       ...
       caption = "lm(trial.mean.correct ~ T.score + Age,data = No.Go.data)") +
...
Inhibition.plot2 <- GnG.performance.merged %>%
...
ggarrange(Inhibition.plot1, Inhibition.plot2)





### Next for Working Memory
# Add the standardized residuals to our original dataframe
Nback.performance.merged$standardized.residuals <- rstandard(Working.Memory.model)

# Add studentized residuals
Nback.performance.merged$studentized.residuals <- rstudent(Working.Memory.model)

# Add the fitted (predicted) values to our original dataframe
Nback.performance.merged$fitted.values <- Working.Memory.model$fitted.values

# Create a graph using the fitted values and the standardizes residuals
Working.Memory.plot1 <- Nback.performance.merged %>%
  ggplot(aes(x = fitted.values, y = standardized.residuals)) +
  geom_point() +
  geom_smooth(method = "lm", color = "black")+
  theme_bw() +
  labs(title = "Standardized Residuals vs Fitted Values\n(Working Memory)",
       x = "Fitted Values",
       y = "Standardized Residuals",
       caption = "lm(target.mean.reacted ~  T.score + Age, data = Target.data)") +
  theme(plot.title = element_text(size = 12,
                                  hjust = 0.5),
        axis.title = element_text(size = 9, face = "bold"),
        axis.text = element_text(size = 9),
...
Working.Memory.plot2 <- Nback.performance.merged %>%
  ggplot(aes(sample= studentized.residuals)) +
  geom_qq() +
  stat_qq_line() +
  theme_bw() +
...
ggarrange(Working.Memory.plot1, Working.Memory.plot2)
```


### Robust regression: bootstrapping 

Since some of our assumptions were violated in the previous sections, we will be using a robust regression boot strapping method with the boot() function in the `boot` packages in r.


```{r running a robus regression}
# Develop a bootstrap function
...
# Run the boostrap function on our regression models
### First on Inhibition
Inhibition.bootResults <- boot(statistic = bootReg, formula = trial.mean.correct ~ T.score + Age,data = No.Go.data, R = 2000)

# Obtain the Confidence Intervals from boot strapping for T.score (index 2)
boot.ci(Inhibition.bootResults, type = "bca", index = 2)

# Obtain the Confidence Intervals from boot strapping for Age(index 3)
boot.ci(Inhibition.bootResults, type = "bca", index = 3)

### Next on Working Memory
Working.Memory.bootResults <- boot(statistic = bootReg, formula = target.mean.reacted ~  T.score + Age, data = Target.data, R = 2000)

# Obtain the Confidence Intervals from boot strapping for T.score (index 2)
boot.ci(Working.Memory.bootResults, type = "bca", index = 2)

# Obtain the Confidence Intervals from boot strapping for Age(index 3)
boot.ci(Working.Memory.bootResults, type = "bca", index = 3)
