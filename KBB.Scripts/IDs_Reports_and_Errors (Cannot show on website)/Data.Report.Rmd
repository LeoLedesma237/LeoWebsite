...
Excluded.Children.NoMatch.org <- read_exce0l("All CFM Excluded Children and Why.xlsx", sheet = "Unable_to_match")
...
recruited.missing.gender <- All.Matched.Siblings %>% select(Date_of_Evaluation, Name_of_the_Village, HOH_First_Name, HOH_Last_Name, Child_First_Name, Child_Last_Name, Child_Date_of_Birth, Child_Gender) %>% filter(!complete.cases(.))

not.recruited.missing.gender <- All.Children %>% select(Date_of_Evaluation, Name_of_the_Village, HOH_First_Name, HOH_Last_Name, Child_First_Name, Child_Last_Name, Child_Date_of_Birth, Child_Gender) %>% filter(!complete.cases(.))

missing.gender <- rbind(recruited.missing.gender,
                        not.recruited.missing.gender) %>% unique()

# set a save directory
setwd("~/KBB_new_2/errors")

write.xlsx(missing.gender,
           file= "missing.gender.xlsx")
```
