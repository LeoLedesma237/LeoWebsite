...
Our next step then is to take only the rows that have at least one data missing and split them into two data frames. One data frame will contain ID's that have rsEEG data and another the other will contain ID's that do not have rsEEG data. We can then use 'fuzzy matching' through the stringdist_join() function, which will match ID's between data frames that look similar to each other. It does this by taking our second data frame (the one with rsEEG data) and comparing the variable we choose from it (ID) to all of the IDs in our first data frame (the ones with missing rsEEG) and calculating a distance number that indicates how similar the ID from the second data frame is to that of the first. If this distance value is close to zero then there is a high likelyhood the ID's actually do match and that will be displayed as an output.
