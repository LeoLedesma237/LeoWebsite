...
ARFA <- read.csv("ARFA.Spelling.Tweaked.Scored.csv")
...
rsEEG <- read.csv("Frequency.Bands.No.Topography.csv")
...
data <- ARFA %>%
  full_join(rsEEG, by = "ID") %>%
  full_join(demo, by = "ID") %>%
...
cat("This is an automated response: The current dataset has the dimensions",paste(dim(data), collapse = " x "),". Below is a frequency table showing the number of missing data for each variable.")

# Create a dataframe of missing data
Missing.data.df <- cbind(colSums(is.na(data))) %>%
...
cat("This is an automated response: For now we are interested in analyzing subjects that have a full dataset. So we will be removing all subjects (rows) that have missing data. In total",nrow(data) - nrow(data2),"subjects were removed, which leaves our n at",nrow(data2),". In this remaining dataset there are",sum(is.na(data2)), "missing data.")
...
# Rearranage the data to have the predictors on top and the EEG outcomes at the bottom. Additionally remove Spelling_Performance
data2 <- data2 %>%
  select(ID,
         Spelling_Error,
         Age,
...
# Describe the Frequency and Percentage of one level of your categorical variables
categorical.predictors <- data2 %>%
  select(Sex,
         Group)

categorical.descriptives.list <- list()


for(ii in 1:length(categorical.predictors)) {

  # Extract the first categorical variable
  categorical.predictor <- categorical.predictors[,ii]
  
  # Obtain the frequency of each level in the variable
  categorical.predictor.count <- plyr::count(categorical.predictor)
  
  # Convert the count into a dataframe
  categorical.predictor.count <- data.frame(categorical.predictor.count)
  
  # Add the total sum (or n)
  categorical.predictor.count$n <- sum(categorical.predictor.count$freq)
  
  # Calculate the percentage for each
  categorical.predictor.count <- categorical.predictor.count %>%
    mutate(percent = round(freq/n,3)*100)

  # Save this in a list
  categorical.descriptives.list[[ii]] <- categorical.predictor.count
  
}


# Combine the outputs in each element of the list into on dataframe
final.categorical.predictors <- categorical.descriptives.list %>%
  do.call(rbind, .)

# Obtain the single value of n from the sum of frequencies in each categorical variable
n <- unique(final.categorical.predictors$n)

# Remove the n as a variable
final.categorical.predictors <- final.categorical.predictors %>%
  select(-n)

# Clean up the table- let's start with the values for the row names
final.categorical.predictors <- final.categorical.predictors %>%
  mutate(x = case_when(
    x == "F" ~ "Female",
    x == "M" ~ "Male",
    x == "BF" ~ "Biological Family",
    x == "IC" ~ "Institutionalized",
    TRUE ~ "Error"
  ))


# Change the names 
names(final.categorical.predictors) <- c("",
                                         "Frequency",
                                         "Percent (%)")
...
final.categorical.predictors %>%
                ...
                kable_classic_2(
                                ...
                                html_font = "Times New Roman") %>%
                row_spec(0,bold=TRUE) %>%
                pack_rows(paste("Sex (n=",n,")",sep=""), 1,2) %>%
                pack_rows(paste("Group (n=",n,")",sep=""),3,4)
...
frequency.bands.descriptives$vars <- c("delta (0.5-4Hz)",
                          "theta (4-8Hz)",
                          "alpha (8-12Hz)",
                          "beta (12-30Hz)",
                          "gamma (30-40Hz)")
...
frequency.bands.descriptives %>%
  ...
  select(`Frequency Band` = vars, 
         ...
         Range = range, 
         n) %>%
  kbl(caption = "Table 3: Descriptive Statistics of Power from each Frequency Band",
      align = "lccc") %>%
...
continous.predictors.outcome <- cbind(continuous.predictors, frequency.bands)

continous.predictors.outcome$Spelling_Error <- as.numeric(continous.predictors.outcome$Spelling_Error)
continous.predictors.outcome$Raw.Score <- as.numeric(continous.predictors.outcome$Raw.Score)
...
ggarrange(histograms.list[[1]], histograms.list[[2]], histograms.list[[3]],
          ...
          histograms.list[[7]], histograms.list[[8]])

# Add it here 
data2 <- data2 %>%
    mutate(delta.log = log2(delta))

data2 %>%
  ggplot(aes(x = delta.log)) +
  geom_histogram() +
  labs(title = "A difference of one means doubleing.")
...
current.variables <- cbind(frequency.bands[ii],continuous.predictors)
...
scatterplot.list[[ii]]  <- plot(cbind(frequency.bands[ii],continuous.predictors))
...
for(ii in 1:length(frequency.bands)) {

# Create the model
model1 <- lm(frequency.bands[[ii]] ~ Spelling_Error + Age + Raw.Score + Male + IC , data2)
...
cleaned.multiple.regression.list[[5]]
```
...
# Create the model
model2 <- lm(frequency.bands[[ii]] ~ Spelling_Error * Age + Raw.Score + Male + IC , data2)
...
cleaned.multiple.regression.interaction.list[[5]]
```
...
data2$Spelling_Error_gmc <- data2$Spelling_Error - mean(data2$Spelling_Error)
data2$Age_gmc <- data2$Age - mean(data2$Age)
data2$Raw.Score_gmc <- data2$Raw.Score - mean(data2$Raw.Score)
...
# Create the model
model3 <- lm(frequency.bands[[ii]] ~ Spelling_Error_gmc * Age_gmc + Raw.Score_gmc + Male + IC , data2)
...
cleaned.multiple.regression.interaction.centered.list[[5]]
```
...
# delta model
delta.model <- lm(delta ~ Spelling_Error_gmc * Age_gmc + Raw.Score_gmc + Male + IC, data = data2)
...
data2 %>%
  mutate(Spelling_Error_factor = ifelse(Spelling_Error <= 4 , "low",
                                        ifelse(Spelling_Error <= 24, "medium", "high"))) %>%
  ggplot(aes(x = Age, y = delta, size = Spelling_Error, color = Spelling_Error_factor)) +
  geom_point(alpha = 0.2) +
  theme_classic()

describe(data2$Spelling_Error)
...
plot3 <- data2 %>%
  ggplot(aes(x = Raw.Score_gmc, y = residuals)) +
  geom_point() +
  geom_smooth(method = "loess",
              se = FALSE) +
...
data2 <- data2 %>%
  mutate(Age_gmc_Quintiles = quintile.category.function(Age_gmc),
         ...
         Raw.Score_gmc_Quintiles = quintile.category.function(Raw.Score_gmc),
...
plot3 <- data2 %>%
  ggplot(aes(x = Raw.Score_gmc_Quintiles, y = residuals)) +
  geom_boxplot() +
  labs(title = "(C) Residuals x CFIT_Raw_Score_gmc") +
  theme_classic()
...
model.no.outliers <- lm(delta ~ Spelling_Error_gmc * Age_gmc + Raw.Score_gmc + Male + IC , data3)
...
```{r visualize beta coefficients, out.width = "100%"}
# test
all_quant <- rq(delta ~ Spelling_Error + Age + Spelling_Error*Age + Raw.Score + Group + Sex, data = data2, tau = seq(from = .05,
                                                                                                        to = .95,
                                                                                                        by = .025))
all_quant_df <- broom::tidy(all_quant)

# All unique terms
unique.terms <- unique(all_quant_df$term)

# Quantile Betas for each predictor
Quantile.beta.plots <- list()

for(ii in 1:length(unique.terms)) {

# Create a graph for the intercept
Quantile.beta.plots[[ii]] <- all_quant_df %>%
  filter(term == unique.terms[ii]) %>%
  ggplot(aes(x = tau, y = estimate)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = .5) +
  geom_line(size = 1) +
  labs(x = "delta power quantile",
       y = paste(unique.terms[ii], "Beta")) +
  theme_light()

}

ggarrange(Quantile.beta.plots[[1]], Quantile.beta.plots[[2]], Quantile.beta.plots[[3]],
          Quantile.beta.plots[[4]], Quantile.beta.plots[[5]], Quantile.beta.plots[[6]],
          Quantile.beta.plots[[7]])
```

### Switching delta to predictor and spelling errors to outcome?

```{r swithing the predictor and outcome}
new.model <- lm(Spelling_Error ~ delta + Age + Raw.Score + Group + Sex, data = data2)
new.model.int <- lm(Spelling_Error ~ delta + Age + delta * Age + Raw.Score + Group + Sex, data = data2)

new.model.list <- list(new.model,
                       new.model.int)

new.model.output.list <- list()

for(ii in 1:2) {

# Create the first model
model <- new.model.list[[ii]]
...
# Creating the foot note for the model
# Create line 1
line.1 <- paste("Residual standard error:",round(model.summary$sigma,4),", on",model$df.residual,"degrees of freedom")

# Create line 2
line.2 <- paste("Multiple R-squared:",round(model.summary$r.squared,5),", Adjusted R-squared: ",round(model.summary$adj.r.squared,5), sep =" ")

# Calculate p value for line 3
regression.p.value <- pf(model.summary$fstatistic[1], model.summary$fstatistic[2], model.summary$fstatistic[3], lower.tail = FALSE)

# Create line 3
line.3 <- paste("F-statistic: ",round(model.summary$fstatistic[1],4)," on ",round(model.summary$fstatistic[2],4)," and ",round(model.summary$fstatistic[3],4)," DF,  p-value:", regression.p.value)


# Convert the model into a data frame
model.df <- broom::tidy(model)

# Split the data frame into term, test-statistics (sorta), and p value
term <- model.df$term
statistics <- model.df %>% select(estimate, std.error, statistic)
p.value <- model.df$p.value

# Clean each variable independently
statistics <- round(statistics,2)
p.value <- round(p.value,3)

# Combine all three into a new data frame for the model
model.cleaned.df <- cbind(term, statistics, p.value)

# Add *** to p values
model.cleaned.df <- model.cleaned.df %>%
  mutate(p.value = case_when(
  p.value < .001 ~ paste("<.001","***",sep=""),
  p.value < .01 ~ paste(round(p.value,2),"**",sep=""),
  p.value < .05 ~ paste(round(p.value,2),"*",sep=""),
  TRUE ~ paste(round(p.value,2))
  
  ))


new.model.output.list[[ii]] <- model.cleaned.df %>%
  kbl(caption = paste("Multiple regression predicting Spelling Error"),
      align = "lccc",
      font = 20) %>%
  ...
  row_spec(0,bold=TRUE) %>%
  footnote(general =  c(line.1, line.2, line.3))

}


new.model.output.list[[1]]
new.model.output.list[[2]]
```
