...
Thus, now we will go to each folder that contains the output of all the scripts and determine how many of our 602 ID's made it to the end.
...
# We are only interested in the ID's that match our 602 sample
for(ii in 1:length(working.directories)) {
  ...
  # Intersect with our 602 IDs of interest
  remaining.IDs <- intersect(rsEEG.IDs, finalData$ID)
...
From our output, we see that we lost a majority of ID's after the first preprocessing step 602 - 519 = 83 because they had no eyes-open marker. Additionally, 2 files did not make it passed the main preprocessing script (script 2) and as expected, there were no issues with the following two scripts. Thus, our final usable rsEEG recordings are 517.
...
After checking the lengths of each recording, 465 rsEEG files made the cut. This will be our final sample size for rsEEG. 
...
# Save the 406 IDs that intersected
write.csv(data.frame(IDs = rsEEG.intersect), file = "Preprocessed.rsEEG.csv")
